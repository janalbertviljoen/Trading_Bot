...
def get_time_resonance(now_lon: dt.datetime) -> Tuple[float, int, str]:
    ...
    if wedge_idx in SNIPER_WEDGES:
        if progress < 12: status = "🔥 INCEPTION"
        elif progress > 88: status = "⌛ CULMINATION"
        else: status = "ACTIVE PULSE"
...
def get_natural_octaves(current_price: float) -> List[float]:
    ...
    return sorted([(base_root + (i * NATURAL_OCTAVE_STEP))**2 for i in range(-12, 13)])
...
app = Dash(__name__, external_stylesheets=[dbc.themes.CYBORG], requests_pathname_prefix=URL_BASE, routes_pathname_prefix=URL_BASE)
...
app.layout = dbc.Container(fluid=True, children=[
    ...
    dbc.Row([
        dbc.Col([
            html.H2(id="hud-price", style={"fontWeight": "900", "fontSize": "64px", "color": "#00ffcc", "lineHeight": "0.8"}),
            html.Div(id="hud-synergy", style={"fontSize": "18px", "color": "#ffcc00", "fontWeight": "bold", "marginTop": "10px"}),
        ], md=4),
        dbc.Col(dbc.Card(dbc.CardBody([
            html.H2(id="hud-objective", className="text-center", style={"fontWeight": "900"}),
            html.Div(id="hud-substatus", className="text-center h5", style={"color": "#00ffcc"})
        ]), id="objective-card", style={"border": "4px solid #444"}), md=5),
        dbc.Col([
            html.Div(id="hud-clock", className="text-end h3", style={"marginBottom": "0px"}),
            html.Div(id="hud-wedge", className="text-end text-info h5", style={"fontWeight": "bold"})
        ], md=3)
    ], className="mt-3 g-2"),
    
    dbc.Row([
        dbc.Col([dbc.Card(dbc.CardBody([html.Div("SNIPER FORECAST (LON)", className="small text-muted mb-2"), html.Div(id="hud-forecast")]), style={"height": "100%"})], md=3),
        dbc.Col(html.Div(id="m-lethality"), md=3), 
        dbc.Col(html.Div(id="m-4h-tide"), md=3), 
        dbc.Col(html.Div(id="m-3m-vel"), md=3),
    ], className="mt-2 g-2"),

    dcc.Graph(id="alpha-chart", style={"height": "75vh"}, config={'displayModeBar': False}),
...
def mk_meter(l, v, color="info", note=""):
    return dbc.Card(dbc.CardBody([
        ...
        html.Div(f"{v:.1f}%", className="h3 mb-0", style={"fontWeight": "bold"}),
...
@app.callback(
    [Output("store-cache", "data"), Output("store-alert-state", "data"),
     Output("hud-price", "children"), Output("hud-synergy", "children"),
     Output("hud-objective", "children"), Output("hud-substatus", "children"),
     Output("objective-card", "style"), Output("hud-clock", "children"),
     Output("hud-wedge", "children"), Output("hud-forecast", "children"),
     Output("m-lethality", "children"), Output("m-4h-tide", "children"), 
...
def update(n, cache_json, alert_state):
    ...
    try:
        if not cache_json:
            ...
            new_df = pd.DataFrame(ohlcv, columns=["ts", "o", "h", "l", "c", "v"]).astype(float)
            new_df["ts"] = pd.to_datetime(new_df["ts"], unit="ms", utc=True).dt.tz_convert(TIMEZONE_LON)
    ...
    def get_rank(series): return series.rolling(200).apply(lambda x: 100*(np.sum(x<=x[-1])/x.size), raw=True).ewm(span=2).mean()
    ...
    if is_fake_spike: alert_state["fake_active"] = True
    ...
    # 3. LETHALITY SCORING
    lethality = 0
    if wedge_idx in SNIPER_WEDGES: lethality += 30
    if in_hg: lethality += 30
    if is_synergy: lethality += 20
    if alert_state["fake_active"] and is_cooled: lethality += 20 # The post-fake reversal logic
    ...
    if is_fake_spike:
        objective, status, bg = "⚠️ DEEP FAKE", "SINGULARITY CLIMAX | NO ENTRY", "#4d3d00"
    elif resonance:
        ...
        if time.time() - alert_state['last_res_ts'] > 1200:
            ...
            requests.post(f"https://api.telegram.org/bot8372044720:AAHx9w6YqaygxgL_vgs_L_JYDGQZGHlxBac/sendMessage", 
    ...
    afig = go.Figure().update_layout(template="plotly_dark", margin=dict(l=10, r=10, t=30, b=10), uirevision="const")
    afig.add_trace(go.Scatter(x=t4h.index, y=t4h, name="4H TIDE", line=dict(color='cyan', width=3)))
    afig.add_trace(go.Scatter(x=t3m.index, y=t3m, name="3m VELOCITY", line=dict(color='yellow', width=1)))
    ...
    for s, lab in zip(levels, labels):
        afig.add_hline(y=s, 
                       line=dict(color="orange" if "BEAT" in lab else "white", dash="dash", width=1), 
                       opacity=0.3, 
                       annotation_text=lab)
    ...
    return [
        cache.to_json(date_format="iso", orient="split"), alert_state,
        f"${price:,.2f}", synergy_text,
        objective, status, {"backgroundColor": bg, "border": "2px solid #00ffcc"}, f"{now_lon:%H:%M:%S}",
        f"WEDGE {wedge_idx} | {time_prog:.1f}%", forecast,
